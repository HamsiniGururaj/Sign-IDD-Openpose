...
class Encoder(nn.Module):
    ...
    def forward(self,
        ...
        #print(f"[ENCODER] Input embed_src: {embed_src.shape}")
        #print(f"[ENCODER] PositionalEncoding + Dropout: {x.shape}")
        for i, layer in enumerate(self.layers):
            #print(f"[ENCODER] Layer {i} input shape: {x.shape}, mask: {None if mask is None else mask.shape}")
            x = layer(x, mask)
